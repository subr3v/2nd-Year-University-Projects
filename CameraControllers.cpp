...
, bMouseYaw(true), bMousePitch(true), CameraSpeed(100.0f), CameraRotationSpeed(5.0f), bMoveKeyboard(true)
{
...
void FBaseInputCameraController::Update(float DeltaTime, class FCamera* Camera)
{
	...
	float YawDifference = (Window->GetWidth() / 2) - CurrentMousePosition.x;
	float PitchDifference = (Window->GetHeight() / 2) - CurrentMousePosition.y;
...
{
	Camera->SetPosition(glm::vec3(0.0f, 10.0f, 75.0f));
	Camera->SetYaw(0.0f);
	Camera->SetPitch(0.0f);
}
